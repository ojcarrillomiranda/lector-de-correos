...
SMTP_HOST = '172.24.1.97'
SMTP_PORT = 587
SMTP_CORREO = 'notificaciones@mct.com.co'
SMTP_PASSWORD = 'temporal_2015'
Emp = ""
...
smtp.starttls()
smtp.login(SMTP_CORREO, SMTP_PASSWORD)

try:
    conn = psycopg2.connect("host='bd.mct.com.co' dbname='switrans' user='admin' password='lITOMUvb7k'")
    print ("conexion a db con exito")
    ...
    def LeerCorreosFactura(self):
        ...
        def setReverseCC():
            ...
            for row in cursor.fetchall():
                ...
                # print "\t*** Dig -> |" + str(stDigCC) + "| Longitud => " + str(len(stDigCC))
                if len(stDigCC) < 3:
                    for x in range(len(stDigCC) + 1, 4):
                        stDigCC = "0" + str(stDigCC)
                stPrefCCReverse = str(r.empresa_codigocontable) + str(stDigCC)
                # print " Key -> " + str(stPrefCCReverse) + " => " + str(r.cencos_codigo)
                dicReverseCC[stPrefCCReverse] = r.cencos_codigo
        setReverseCC()

        def enviaCorreoConfirmacion(cod_documento, modulo):
            #enviamos correo informando se han cargado las imagenes
            modulo_string = ""
            if modulo == 120:
                modulo_string = "la factura: "
            mensaje = MIMEText("""El cargue de las facturas para """ + str(modulo_string) + str(cod_documento) + """  fue realizado correctamente. Puede ingresar a Switrans y verificar.\n\nCordialmente,\n\n\nEquipo TICs MCT.""")
            mensaje['From']="notificaciones@mct.com.co"
            mensaje['To']= str(emailFrom)
            mensaje['Subject']="OK " + str(email_message['Subject'])
            smtp.sendmail("notificaciones@mct.com.co",
                str(emailFrom),
                mensaje.as_string())

        def enviaCorreoError(cod_documento, modulo, filename, msg):
            #enviamos correo informando se han cargado las imagenes
            modulo_string = ""
            if modulo == 120:
                modulo_string = "la factura: "
            mensaje = MIMEText("""El cargue de la factura """ + str(cod_documento) + """ ha generado un ERROR y NO fue realizado.\n\nArchivo: """ + str(filename) + """\n\n""" +msg+ """\n\nCordialmente,\n\n\nEquipo TICs MCT.""")
            mensaje['From']="notificaciones@mct.com.co"
            mensaje['To']= str(emailFrom) + ",logs.switrans@mct.com.co"
            mensaje['Subject']="Error Factura: " + str(cod_documento) + " - " + str(email_message['Subject'])
            smtp.sendmail("notificaciones@mct.com.co",emailFrom,
                # ["facturacion@mct.com.co", "logs.switrans@mct.com.co", "erney.vargas@mct.com.co"],
                mensaje.as_string())
            Msg = "\033[92menviando correo de ERROR de cargue\033[0m " + emailFrom
            print (Msg)
            ...
            for part in email_message.walk():
                ...
                try:
                    ...
                    if(existeDigitalizado):
                        ...
                        print(msg)
                        enviaCorreoError(cod_documento, modulo, filename, msg)
                        break
                except psycopg2.Error as e:
                    print (e.pgerror)
                    return False
                #filename = str(facturaSubj) + '_' + str(ano) + str(mes) + str(dia) + str(sem) + '.' + str(file_type)
                ...
                # insertando en base de datos switrans
                # digitalizado_ruta = '/usr/local/apache/htdocs/switrans/images/facturas/'
                InsertFactura = (
                ...
                if errorFile == 1:
                    ...
                    enviaCorreoConfirmacion(cod_documento, modulo)
        ...
        for num in data[0].split():
            ...
            if typ == 'OK':
                ...
                Msg = " Cantidad es " + str(counteraux) + " subject :" + Subject + " From : " + From
                print(Msg)
                ...
                if emailSubjectCheck(Subj) is False:
                    print("El Asunto no es correcto")
                    break
                else:
