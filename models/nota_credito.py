...
class NotaCredito(object):
    ...
    def leer_correos_nota_credito(self, conex):
        cursor = conex.cursor()
        conexion = conex
        Funcion().setReverseCC(cursor)
        def get_datos_nota_credito(empCodigo, codCcNotaCredito):
            print ("empresa: ", empCodigo)
            print (codCcNotaCredito)
            global detach_dir, detach_dir1, codigo_nota, cencos_nota, fecha_nota, ano, mes, dia, notaSubj
            NotaSQL = "Select notcon_codigo as nota_codigo, cencos_codigo, notcon_fechacreacion AS nota_fechacreacion from tb_notacontabilidad where empresa_codigo = '" + str(
                empCodigo) + "' AND notcon_cencoscodigo = '" + str(codCcNotaCredito) + "'"
            Msg = "Consulta de la nota credito Empresa " + str(empCodigo) + " DigCCResolucion " + str(
                codCcNotaCredito) + " ejecutada con éxito"
            try:
                print (NotaSQL)
                ...
                for row in cursor.fetchall():
                    ...
                    ruta_servidor = '/documents/facturas/' + str(Emp) + '/' + str(ano) + '/' + str(cencos_nota) + '/' + str(mes) + '/'
                    digitalizado_ruta = '/usr/local/apache/htdocs/switrans/images/facturas/' + str(Emp) + '/' + str(
                        ano) + '/' + str(cencos_nota) + '/' + str(mes) + '/'
                    ContAux = ContAux + 1
                if ContAux > 0:
                    datos_nota_credito = {
                        "codigo_nota": codigo_nota,
                        "cencos_nota": cencos_nota,
        ...
        def update_nota_credito():
            updateNotaCredito = (
                    ...
                    cod_documento))
            try:
                cursor.execute(updateNotaCredito)
                conexion.commit()
                ...
                logfile.write(Msg)
                print Msg
                print("ERROR FATAL EN LA CONSULTA " + str(updateNotaCredito))
                errorFile = 1
